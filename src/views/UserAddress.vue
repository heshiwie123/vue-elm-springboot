...
function listDeliveryAddressByUserId() {
  axios.get(`deliveryAddress/listDeliveryAddressByUserId?userId=${user.value.userId}`).then(response => {
    deliveryAddressArr.value = response.data.data.deliveryAddresses;
  }).catch(error => {
    console.error(error);
  });
}
...
//把用户选择的默认送货地址存储到localStorage中
  userSession.setLocalStorage(user.value.userId, deliveryAddress);
...
function removeUserAddress(daId) {
  ...
  axios.delete(`deliveryAddress/removeDeliveryAddress?daId=${daId}`).then(response => {
    if (response.data.code === 200) {
      let deliveryAddress = userSession.getLocalStorage(user.value.userId);
      if (deliveryAddress != null && deliveryAddress.daId === daId) {
        userSession.removeLocalStorage(user.value.userId);
      }
      listDeliveryAddressByUserId();
    } else {
      alert('删除地址失败！');
